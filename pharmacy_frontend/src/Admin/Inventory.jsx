...
const Inventory = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:8086/drug');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingProduct) {
        await axios.put(`http://localhost:8086/drug/update/${editingProduct.id}`, formData, config);
        setEditingProduct(null);
      } else {
        await axios.post('http://localhost:8086/drug/add', formData, config);
      }
  ...
  const handleDelete = async (id) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`http://localhost:8086/drug/delete/${id}`, {
        headers: { Authorization: token ? `Bearer ${token}` : undefined },
      });
      fetchProducts();
    } catch (err) {
  ...
  const handleViewSupplier = async (supplierId) => {
    ...
    try {
      const res = await axios.get(`http://localhost:8085/supplier/${supplierId}`, {
        headers: { Authorization: token ? `Bearer ${token}` : undefined },
      });
