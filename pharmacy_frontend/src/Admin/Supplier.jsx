...
const Supplier = () => {
  ...
  const fetchSuppliers = async () => {
    setLoading(true);
    try {
      const res = await axios.get('http://localhost:8085/supplier');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingSupplier) {
        await axios.put(`http://localhost:8085/supplier/${editingSupplier.id}`, formData);
        setEditingSupplier(null);
      } else {
        await axios.post('http://localhost:8085/supplier', formData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    setLoading(true);
    try {
      await axios.delete(`http://localhost:8085/supplier/${id}`);
      fetchSuppliers();
    } catch (error) {
