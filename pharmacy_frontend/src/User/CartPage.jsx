...
const CartPage = () => {
  ...
  const updateQuantity = async (drugId, currentQuantity, type) => {
    try {
      ...
      let url = "";
      if (type === "increase") {
        url = `http://localhost:8084/cart/increase/${drugId}`;
      } else if (type === "decrease") {
        url = `http://localhost:8084/cart/decrease/${drugId}`;
      } else {
        return;
      }
  ...
  const removeItem = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(`http://localhost:8084/cart/decrease/${id}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const clearCart = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await fetch("http://localhost:8084/cart/clear", {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const placeOrder = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:8083/orders/place?paymentMethod=${encodeURIComponent(paymentMode)}`,
        {
          method: "POST",
          headers: {
