  ...
  useEffect(() => {
    ...
    const fetchOrderAndPayment = async () => {
      ...
      try {
        ...
        let orderData = order;
        if (!orderData) {
          const orderRes = await fetch(`http://localhost:8083/orders/${orderId}`, {
            headers: {
        ...
        const paymentRes = await fetch(`http://localhost:8085/payment/link/${orderId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        if (!paymentRes.ok) throw new Error("Failed to fetch payment link");
        const paymentData = await paymentRes.json();
        setPaymentLink(paymentData.link || paymentData.url || "");
      } catch (err) {
    ...
    if (orderId) fetchOrderAndPayment();
